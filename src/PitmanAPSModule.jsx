...
import { Activity, CheckCircle2, Share2, X, FileCheck, ArrowLeft, Eye, Clock, Maximize, Minimize, TrendingUp, Search, Lock } from 'lucide-react';
                                    ...
                                    {paginatedList.map((test, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {/* Icon */}
                                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isLockedForUser ? 'bg-gray-200' : 'bg-indigo-50'}`}>
                                                    {isLockedForUser ? <Lock className="w-5 h-5 text-gray-500" /> : <Eye className="w-5 h-5 text-indigo-500" />}
                                                </div>

                                                {/* Title + chips */}
                                                <div className="flex-1 min-w-0">
                                                    <h3 className={`font-black text-sm md:text-base transition-colors truncate ${isLockedForUser ? 'text-gray-500' : 'text-gray-900 group-hover:text-[#0d6e70]'}`}>{test.title}</h3>
                                                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                                        ...
                                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isLockedForUser ? 'bg-gray-200 text-gray-500' : 'bg-blue-50 text-blue-600'}`}>{wordCount} words</span>
